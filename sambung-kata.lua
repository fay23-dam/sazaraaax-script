...
-- Fitur: Auto play, Select Word, SimpleSpy, Status Loading
-- WindUI Build
-- =========================================================
...
local function onMatchUI(cmd, value)
    ...
    elseif cmd == "UpdateServerLetter" then
		...
		-- 🔥 PAKSA REFRESH SELECT MODE
		if getWordsEnabled then
			task.spawn(function()
				task.wait(0.05)
				updateWordButtons()
			end)
...
function updateWordButtons()
    ...
    if not serverLetter or serverLetter == "" then return end
    if #kataModule == 0 then return end
    ...
    -- 🔥 PENTING: pastikan array string murni
    wordDropdown:Refresh(limited)

    -- Select default
    task.wait() -- beri 1 frame delay supaya UI refresh dulu
    wordDropdown:Select(limited[1])
    selectedWord = limited[1]
	print("MATCH:", matchActive)
	print("TURN:", isMyTurn)
	print("LETTER:", serverLetter)
	print("WORDS:", #kataModule)
	print("SERVER LETTER:", serverLetter)
...
wordDropdown = SelectTab:Dropdown({
    ...
    Callback = function(option)
        -- 🔥 WindUI kadang kirim string langsung, bukan table
...
submitButton = SelectTab:Button({
    ...
    Callback = function()
        if not getWordsEnabled then 
            notify("⚠️", "Mode Select Word tidak aktif", 2)
            return 
        end
        if not isMyTurn then 
            notify("⚠️", "Bukan giliran Anda", 2)
            return 
        end
        if not selectedWord then 
            notify("⚠️", "Pilih kata terlebih dahulu", 2)
            return 
        end
        if serverLetter == "" then 
            notify("⚠️", "Huruf awal kosong", 2)
            return 
        end
        -- Perbaikan: pastikan kata sesuai dengan huruf awal
        if string.sub(selectedWord, 1, #serverLetter) ~= serverLetter then
            notify("⚠️", "Kata tidak sesuai dengan huruf awal", 2)
            return
        end
        -- Cek apakah kata sudah digunakan atau blacklist
        if isUsed(selectedWord) or wrongWordsSet[selectedWord] or blacklistedWords[selectedWord] then
            notify("⚠️", "Kata tidak valid (sudah dipakai/blacklist)", 2)
            return
        end
...
local function updateConfigDisplay()
    configDisplay:SetDesc(
        ...
        " | Select: "..tostring(getWordsEnabled)..
...
local function loadConfig()
    ...
    getWordsEnabled = decoded.getWordsEnabled ~= nil and decoded.getWordsEnabled or getWordsEnabled
    maxWordsToShow = decoded.maxWordsToShow or maxWordsToShow

    if autoToggle then autoToggle:Set(autoEnabled) end
    if getWordsToggle then getWordsToggle:Set(getWordsEnabled) end
...
AboutTab:Paragraph({
    ...
    Desc = "Auto Kata (Optimized)\nVersi: 6.0 (No Aggression/Length)\nby sazaraaax & danz\nFitur: Auto play, Select Word, SimpleSpy, save/load config",
    Color = "Blue",
})
