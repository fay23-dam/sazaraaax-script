...
local function buildSkipItemsUI()
    ...
    local skipListLayout = Instance.new("UIListLayout")
    skipListLayout.Padding = UDim.new(0, 4 * scaleFactor)
    skipListLayout.SortOrder = Enum.SortOrder.LayoutOrder
    skipListLayout.Parent = skipListFrame

    local function addCheckboxRow(itemName)
        local row = Instance.new("Frame")
        row.Size = UDim2.new(1, -16 * scaleFactor, 0, 32 * scaleFactor)
        ...
        local function updateCheckboxImage()
            if checkboxState then
                ...
                check.Image = "rbxassetid://0" -- kotak kosong (ganti jika ada)
...
-- SYSTEM SECTION
-- ============================================================
makeSectionLabel(sfFarm, "// system", 24)
local btnReset, _ = makeSmallBtn(sfFarm, "// reset counter", Color3.fromRGB(220, 60, 60), 25)

makeDivider(sfFarm, 26)
...
makeSectionLabel(sfFarm, "// community", 27)
local btnDiscord = makeLinkBtn(sfFarm, "D", "Discord", "// discord.gg/HB9gqZGMnT", Color3.fromRGB(114, 137, 218), 28)
local btnWA = makeLinkBtn(sfFarm, "W", "WhatsApp Channel", "// wa channel", Color3.fromRGB(37, 211, 102), 29)
local btnSaweria = makeLinkBtn(sfFarm, "c", "Saweria", "// saweria.co/sazaraaa", Color3.fromRGB(255, 185, 55), 30)
...
makeSmallBtn(sfStats, "// clear log", Color3.fromRGB(220, 60, 60), 9).MouseButton1Click:Connect(function()
...
local function createESP(slotChild)
    if espLabels[slotChild] then return end
    local lootUI, adornPart = findLootData(slotChild)
    if not lootUI or not adornPart then return end
    local name = getItemName(lootUI)
    if not name then return end
    if isItemOnBase(adornPart) then return end

    local accentCol = RARITY_COLORS[name] or DEFAULT_ESP_COLOR
    ...
    espLabels[slotChild] = { billboard = billboard, distLabel = distLabel, adornPart = adornPart, name = name }
end

local function removeESP(item)
    if espLabels[item] then
        espLabels[item].billboard:Destroy()
        espLabels[item] = nil
    end
end

-- Throttled distance updates (every 0.2 seconds)
    ...
    if now - lastDistanceUpdate >= 0.2 then
        ...
        for item, data in pairs(espLabels) do
            if not item.Parent then
                ...
                local adornPart = data.adornPart
                if not adornPart or not adornPart.Parent then
                    removeESP(item)
                elseif not isItemOnBase(adornPart) then
                    local dist = (humanoidRootPart.Position - adornPart.Position).Magnitude
                    data.distLabel.Text = string.format("%.1f studs", dist)
                    if dist < 20 then
                        data.distLabel.TextColor3 = Color3.fromRGB(52, 199, 109)
                    elseif dist < 50 then
                        data.distLabel.TextColor3 = Color3.fromRGB(255, 185, 55)
                    else
                        data.distLabel.TextColor3 = Color3.fromRGB(220, 80, 80)
                    end
                else
                    removeESP(item)
                end
...
-- ============================================================
-- CACHE LOOT ITEMS UNTUK OPTIMASI
-- ============================================================
local cachedLootItems = {} -- array { item = child, adornPart = part, name = name }
...
-- Initial scan untuk mengisi cache dan ESP
local function initialScan()
    for _, slot in ipairs(WorldFolder:GetChildren()) do
        for _, child in ipairs(slot:GetChildren()) do
            ...
            task.wait() -- sedikit jeda agar tidak overload
        end
    end
end
...
-- ============================================================
-- PHASE 2: AUTO COLLECT (menggunakan cachedLootItems)
-- ============================================================
...
btnNearestLoot.MouseButton1Click:Connect(function()
    ...
    for item, data in pairs(espLabels) do
        if data.adornPart and data.adornPart.Parent then
