--[[
    ...
    by sazaraaax & dhanzy
    Fixed: ESP toggle, dropItem nil, collect return to base, auto-collect snapshot (no skipping)
    Modified: All interaction uses E key, drop uses G key. UI Button E & G always visible for Android/PC.
    Note: Game's default jump & run buttons are NOT removed.
--]]
...
-- Selalu pakai keyboard E (interact) dan G (drop)
-- UI Button di layar tersedia untuk semua platform (Android / PC)
-- Tombol lompat & lari bawaan game TIDAK dihapus
-- ============================================================

--- Kirim tombol E ke game (berlaku di semua platform)
...
--- Kirim tombol G ke game (berlaku di semua platform)
...
--- Interaksi dengan part – selalu E
local function interactWithPart(_part)
    pressE()
    return true
end

--- Jatuhkan item – selalu G
local function dropItem()
    pressG()
    return true
end

--- Pakai bat di slot tertentu (slot 1-4)
...
-- ============================================================
-- ON-SCREEN UI BUTTON E & G
-- Tombol ini muncul di layar untuk semua platform.
-- Pemain Android bisa tap; pemain PC juga bisa klik.
-- Posisi: kanan bawah, di atas kontrol bawaan game.
-- Tombol jump & run bawaan game TIDAK disentuh.
-- ============================================================
local function createActionButtonsGui()
    -- Hapus GUI lama jika ada
    local old = playerGui:FindFirstChild("VictoriaActionBtns")
    if old then old:Destroy() end

    local gui = Instance.new("ScreenGui")
    gui.Name = "VictoriaActionBtns"
    ...
    local sf = math.clamp(vp.X / 1920, 0.7, 1.4)
    local BTN = 64 * sf  -- ukuran tombol
    local PAD = 12 * sf  -- jarak antar tombol
    -- Posisi: kiri bawah, cukup tinggi agar tidak overlap jump/run bawaan
    local bottomY = -160 * sf
    local leftX   =  20 * sf

    local function makeBtn(label, posX, posY, accentColor, callback)
        local btn = Instance.new("TextButton")
        btn.Size = UDim2.new(0, BTN, 0, BTN)
        btn.Position = UDim2.new(0, posX, 1, posY - BTN)
        ...
        btn.MouseButton1Down:Connect(function()
            TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundColor3 = Color3.fromRGB(30, 35, 30)}):Play()
    ...
    -- Tombol E (Interact/Collect) - warna hijau
    makeBtn(
        "E",
        leftX,
        bottomY,
        Color3.fromRGB(52, 199, 109),
        function()
            pressE()
            addLog("E (interact) ditekan via UI", Color3.fromRGB(52, 199, 109))
        end
    )

    -- Tombol G (Drop) - warna oranye, di sebelah kanan E
    makeBtn(
        "G",
        leftX + BTN + PAD,
        bottomY,
        ...
        function()
            pressG()
            addLog("G (drop) ditekan via UI", Color3.fromRGB(255, 140, 50))
        end
    )

    -- Label kecil di bawah tombol agar jelas fungsinya
    local labelE = Instance.new("TextLabel")
    labelE.Size = UDim2.new(0, BTN, 0, 14 * sf)
    labelE.Position = UDim2.new(0, leftX, 1, bottomY + 4 * sf)
    labelE.BackgroundTransparency = 1
    labelE.Text = "interact"
    labelE.TextColor3 = Color3.fromRGB(52, 199, 109)
    labelE.TextScaled = true
    labelE.Font = Enum.Font.Code
    labelE.ZIndex = 10
    labelE.Parent = gui

    local labelG = Instance.new("TextLabel")
    labelG.Size = UDim2.new(0, BTN, 0, 14 * sf)
    labelG.Position = UDim2.new(0, leftX + BTN + PAD, 1, bottomY + 4 * sf)
    labelG.BackgroundTransparency = 1
    labelG.Text = "drop"
    labelG.TextColor3 = Color3.fromRGB(255, 140, 50)
    labelG.TextScaled = true
    labelG.Font = Enum.Font.Code
    labelG.ZIndex = 10
    labelG.Parent = gui
...
local function switchTab(name)
    currentTab = name
    for tabName, sf in pairs(scrollFrames) do sf.Visible = (tabName == name) end
...
for _, data in pairs(v1) do
    if data.name then allItemNames[data.name] = true end
end
...
local function buildSkipItemsUI()
    ...
    local sortedNames = {}
    for name in pairs(allItemNames) do table.insert(sortedNames, name) end
    table.sort(sortedNames)
    for _, name in ipairs(sortedNames) do addCheckboxRow(name) end
end
buildSkipItemsUI()
...
local function saveOriginal(obj, prop, value)
    ...
    if originalData[obj][prop] == nil then originalData[obj][prop] = value end
end
...
local function processWorkspace()
    ...
    for i = 1, #all, CONFIG.BATCH_SIZE do
        for j = i, math.min(i + CONFIG.BATCH_SIZE - 1, #all) do processObject(all[j]) end
        task.wait()
    end
end

local function hookNewObjects()
    table.insert(potatoConnections, workspace.DescendantAdded:Connect(function(obj)
        task.defer(function() if potatoMode then processObject(obj) end end)
    end))
end

local function clearConnections()
    for _, c in ipairs(potatoConnections) do pcall(function() c:Disconnect() end) end
    potatoConnections = {}
end
...
local function toggleExtremePotato()
    ...
    if potatoMode then
        ...
        pcall(function() settings().Rendering.QualityLevel = originalQuality or Enum.QualityLevel.Level03 end)
...
potatoBtn.MouseButton1Click:Connect(toggleExtremePotato)

-- ============================================================
-- ESP CATEGORIES
-- ============================================================
makeDivider(sfFarm, 24)
makeSectionLabel(sfFarm, "// esp categories", 25)

local espListFrame = Instance.new("ScrollingFrame")
espListFrame.Size = UDim2.new(1, -24 * scaleFactor, 0, 120 * scaleFactor)
espListFrame.BackgroundColor3 = Color3.fromRGB(20, 22, 24)
espListFrame.BorderSizePixel = 0
espListFrame.ZIndex = 3
espListFrame.LayoutOrder = 26
espListFrame.Parent = sfFarm
espListFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
espListFrame.CanvasSize = UDim2.new(0, 0, 0, 0)
Instance.new("UICorner", espListFrame).CornerRadius = UDim.new(0, 8 * scaleFactor)
Instance.new("UIStroke", espListFrame).Color = Color3.fromRGB(38, 43, 48)

local espListLayout = Instance.new("UIListLayout")
espListLayout.Padding = UDim.new(0, 4 * scaleFactor)
espListLayout.SortOrder = Enum.SortOrder.LayoutOrder
espListLayout.Parent = espListFrame

local espPad = Instance.new("UIPadding")
espPad.PaddingTop = UDim.new(0, 6 * scaleFactor)
espPad.PaddingBottom = UDim.new(0, 6 * scaleFactor)
espPad.PaddingLeft = UDim.new(0, 8 * scaleFactor)
espPad.PaddingRight = UDim.new(0, 8 * scaleFactor)
espPad.Parent = espListFrame

local function addESPCategoryRow(category, displayName, accentColor)
    local row = Instance.new("Frame")
    row.Size = UDim2.new(1, 0, 0, 32 * scaleFactor)
    row.BackgroundColor3 = Color3.fromRGB(28, 32, 36)
    row.BorderSizePixel = 0
    ...
    btn.MouseButton1Click:Connect(function()
        espCategories[category] = not espCategories[category]
        updateCheckboxImage()
        refreshAllESP()
        addLog("ESP " .. displayName .. " " .. (espCategories[category] and "ON" or "OFF"), accentColor)
    end)
end

addESPCategoryRow("items",    "Items (loot)", Color3.fromRGB(80, 160, 230))
addESPCategoryRow("npcs",     "NPCs",         Color3.fromRGB(0, 200, 100))
addESPCategoryRow("monsters", "Monsters",     Color3.fromRGB(220, 60, 60))
...
function refreshAllESP()
    ...
    if espCategories.monsters then
...
WorldFolder.DescendantAdded:Connect(function(desc)
    task.defer(function()
        if not espCategories.items then return end
...
WorldFolder.DescendantRemoving:Connect(function(desc) removeESP(desc) end)

NPCModels.ChildAdded:Connect(function(npc)
    task.defer(function()
        if not espCategories.npcs then return end
        if npc:IsA("Model") then
            local head = npc:FindFirstChild("Head")
...
NPCModels.ChildRemoved:Connect(function(npc) removeESP(npc) end)
...
if monstersContainer then
    monstersContainer.ChildAdded:Connect(function(monster)
        if not espCategories.monsters then return end
        if monster:IsA("Model") then
            local head = monster:FindFirstChild("Head")
    ...
    monstersContainer.ChildRemoved:Connect(function(monster) removeESP(monster) end)
end
    ...
    if now - lastDistanceUpdate >= 0.2 then
        ...
        for obj, data in pairs(espLabels) do
            if not data.adornPart or not data.adornPart.Parent then
                ...
                if bg then
                    local distLabel = bg:FindFirstChild("Distance")
                    if distLabel then
...
local function updateCachedItem(child)
    ...
    if lootUI and adornPart then
        ...
        if name then
            ...
            for i, v in ipairs(cachedLootItems) do
                if v.item == child then
                    found = true; v.adornPart = adornPart; v.name = name; break
                end
            end
            if not found then
        ...
        for i, v in ipairs(cachedLootItems) do
            if v.item == child then table.remove(cachedLootItems, i); break end
        end
    end
end
...
WorldFolder.DescendantRemoving:Connect(function(desc)
    for i, v in ipairs(cachedLootItems) do
        if v.item == desc then table.remove(cachedLootItems, i); break end
    end
    removeESP(desc)
end)
...
local function runCollectLoots()
    ...
    local lootSnapshot = {}
    for _, loot in ipairs(cachedLootItems) do
        table.insert(lootSnapshot, loot)
    end

    for i, loot in ipairs(lootSnapshot) do
        if not isCollectingActive then break end

        if not loot.item.Parent then
            addLog("item no longer exists: " .. loot.name, Color3.fromRGB(255, 185, 55))
            goto continue
        end
        if not loot.adornPart or not loot.adornPart.Parent then
            addLog("adorn part missing for: " .. loot.name, Color3.fromRGB(255, 185, 55))
            goto continue
        end
        if isItemOnBase(loot.adornPart) then
            addLog("item on base, skipping: " .. loot.name, Color3.fromRGB(255, 185, 55))
            goto continue
        end
        ...
        setStatus("collecting " .. i .. "/" .. #lootSnapshot .. ": " .. loot.name, Color3.fromRGB(80, 160, 230))
        ...
        interactWithPart(loot.adornPart)  -- tekan E
        task.wait(d.afterE)
        stopFly()
    ...
    for _, monster in ipairs(monsters:GetChildren()) do
        if monster:IsA("Model") then
            ...
            if head and head:IsA("BasePart") then
                ...
                if dist < nearestDist then nearestDist = dist; nearest = {model = monster, head = head} end
            end
        end
    end
    return nearest
...
local function runAutoMonster()
    ...
    while isMonsterActive do
        local batUsed = false
        for slot = 1, 4 do
            if useBatInSlot(slot) then batUsed = true; break end
        end
        if not batUsed then
...
btnMonster.MouseButton1Click:Connect(function()
    if isMonsterActive then isMonsterActive = false
    else task.spawn(runAutoMonster) end
end)
...
btnReset.MouseButton1Click:Connect(function()
    totalCollected = 0; totalOpened = 0; totalSkipped = 0
    sessionStart = tick()
    updateCounter()
...
btnDiscord.MouseButton1Click:Connect(function() setStatus("discord.gg/HB9gqZGMnT", Color3.fromRGB(114, 137, 218)) end)
btnWA.MouseButton1Click:Connect(function() setStatus("whatsapp.com/channel/0029VbCBSBOCRs1pRNYpPN0r", Color3.fromRGB(37, 211, 102)) end)
btnSaweria.MouseButton1Click:Connect(function() setStatus("saweria.co/sazaraaa", Color3.fromRGB(255, 185, 55)) end)

-- Update NPC button text
local function updateNPCButton()
    local children = NPCModels:GetChildren()
    local mainLbl, subLbl = nil, btnNPC:FindFirstChild("Sub")
...
RunService.Heartbeat:Connect(function()
    if #logEntries > lastLogCount then
        for i = lastLogCount + 1, #logEntries do renderLog(logEntries[i]) end
