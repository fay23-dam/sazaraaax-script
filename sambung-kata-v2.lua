...
for _, word in ipairs(kataModule) do
    ...
    if not wordsByFirstLetter[first] then
        wordsByFirstLetter[first] = {}
    end
    table.insert(wordsByFirstLetter[first], word)
end
...
local function notify(title, message, time)
    Rayfield:Notify({ Title = title, Content = message, Duration = time or 2.5 })
end
